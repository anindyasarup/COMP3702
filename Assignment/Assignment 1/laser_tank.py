...
This file contains a class representing Laser Tank game map. You should make 
use of this class in your solver. 

Running this file directly with a valid testcase file given as an argument 
launches an interactive instance of the Laser Tank game. 
...
class LaserTankMap:
    ...
    # render characters
    MAP_GLYPH_TABLE = {LAND_SYMBOL: '   ', WATER_SYMBOL: 'WWW',
                       OBSTACLE_SYMBOL: 'XXX', BRIDGE_SYMBOL: '[B]',
                       BRICK_SYMBOL: '[K]', ICE_SYMBOL: '-I-',
                       TELEPORT_SYMBOL: '(T)', FLAG_SYMBOL: ' F ',
                       MIRROR_UL_SYMBOL: ' /|', MIRROR_UR_SYMBOL: '|\\ ',
                       MIRROR_DL_SYMBOL: ' \\|', MIRROR_DR_SYMBOL: '|/ ',
                       ANTI_TANK_UP_SYMBOL: '[U]', ANTI_TANK_DOWN_SYMBOL: '[D]',
                       ANTI_TANK_LEFT_SYMBOL: '[L]',
                       ANTI_TANK_RIGHT_SYMBOL: '[R]',
                       ANTI_TANK_DESTROYED_SYMBOL: '[X]'}
    ...
    def __init__(self, x_size, y_size, grid_data, player_x=None, player_y=None, player_heading=None):
        ...
        :param grid_data:
        matrix with dimensions (y_size, x_size) where each element is a valid
        symbol
        """
        self.x_size = x_size
        self.y_size = y_size
        ...
        if player_x is None and player_y is None and player_heading is None:
            ...
            for i in range(y_size):
                ...
                if found:
                    break
            if not found:
                raise Exception("LaserTank Map Error: Grid data does not "
                                "contain player symbol")
        elif player_x is None or player_y is None or player_heading is None:
            raise Exception("LaserTank Map Error: Incomplete player "
                            "coordinates given")
        else:
    ...
    def process_input_file(filename):
        """
        Process the given input file and create a new map instance based on
        the input file.
        ...
        row_len = len(rows[0])
        for row in rows:
            assert len(row) == row_len, "LaserTank Map Error: Mismatch in " \
                                        "row length "
    ...
    def apply_move(self, move):
        ...
        :param move: self.MOVE_FORWARD, self.TURN_LEFT, self.TURN_RIGHT or
        self.SHOOT_LASER
        :return: LaserTankMap.SUCCESS if move was successful and no collision
        (or no effect move) occurred, LaserTankMap.COLLISION if the move
        resulted collision or had no effect, LaserTankMap.GAME_OVER if the move
        resulted in game over
        """
    ...
    def cell_is_laser_movable(self, y, x, heading):
        """
        Check if the tile at coordinated (y, x) is movable by a laser with
        the given heading. :param y: y coord :param x: x coord :param
        heading: laser direction :return: True is movable, false otherwise
        """
...
def main(arglist):
    ...
    if len(arglist) != 1:
        print("Running this file directly launches a playable game of "
              "LaserTank based on the given map file.")
        print("Usage: laser_tank.py [map_file_name]")
        return

    print("Use W to move forward, A and S to turn. Use (spacebar) to shoot. "
          "Press 'q' to quit." +
